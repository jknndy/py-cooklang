...
def test_extract_images_multiple():
    parser = CooklangParser()
    text = (
        "This is a test recipe with multiple images "
        "![First dish](images/first.jpg) and ![Second dish](images/second.jpg)"
    )
    result = parser.extract_images(text)
    assert result == [
        {"description": "First dish", "path": "images/first.jpg"},
...
def test_extract_images_url():
    ...
    result = parser.extract_images(text)
    assert result == [
        {"description": "Dish", "path": "http://example.com/images/dish.jpg"}
    ]
...
def test_extract_images_mixed_content():
    ...
    result = parser.extract_images(text)
    assert result == [
        {"description": "Dish1", "path": "images/dish1.jpg"},
